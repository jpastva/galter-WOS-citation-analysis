...
def analyze_package(package_dir, output_dir):
    df = pd.read_pickle(package_dir + '/concatenated.pkl')
...
def main():
    ...
    match len(args):
        case 3:
            ...
            output_dir = args[2]
        case 2:
            package_dir = args[1] # assume the argument is the input path
            output_dir = "."    # default output path is current working directory
        case 1:
            package_dir = "."     # current working directory
            output_dir = "."    # current working directory
        case _:
            print("Usage: python WOS_citation_analysis.py <input_path> <output_path>")
            sys.exit(1)

    analyze_package(package_dir, output_dir)

    print("Done, find analysis outputs in " + output_dir) 
